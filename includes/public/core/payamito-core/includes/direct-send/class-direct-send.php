...
if (!class_exists("Payamito_Direct_Send")) {
  ...
  class Payamito_Direct_Send
  {
    ...
    private function init()
    {
      ...
      if (!self::IsDirectSendPage()) return;
    ...
    private function patternSend($mobileNumbers, $patterns, $patternID)
    {
      ...
      foreach ($mobileNumbers as $mobile) {
        $reslut = payamito_send_pattern($mobile, $preparedPattern, $patternID, $this->slug);
        wp_send_json(array('logShow'=>'1','result'=>payamito_code_to_message($reslut) ));
        die;
      }
    }
    private function textSend($mobileNumbers, $text,$senderNumber)
    {
      $reslut = payamito_group_send($mobileNumbers, $text, $senderNumber,$this->slug);
      if ($reslut != 1) {
        $m=__("Sending canceled because %s","payamito");
        self::ajax_response('error', sprintf($m, payamito_code_to_message($reslut)), __("!Error", "payamito"), true);
      }else{
        wp_send_json(array('logShow'=>'1','result'=>payamito_code_to_message($reslut) ));
      }
    }
    ...
    {
      wp_send_json(array('type' => $type, 'message' => $message, "title" => $title, 'exit' => $exit));
      die;
    }
      ...
      $awesome_support_sms_settings = array(
        ...
        'fields' =>  array(
          ...
          array(
            ...
            'dependency' => array("payamito_direct_send_active_pattern", '==', 'true'),
          ),
          array(
 ...
 Payamito_Direct_Send::get_instance();