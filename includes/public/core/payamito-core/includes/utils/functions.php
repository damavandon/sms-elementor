...
if (!function_exists('payamito_send_pattern')) {
	...
	function payamito_send_pattern($to, $text, $bodyid, $slug)
	{
		...
		Payamito_DB::insert_sms($to, 1, $slug, $status, serialize($text));
...
if (!function_exists('payamito_group_send')) {
	...
	function payamito_group_send(array $to, string $text, $sendernumber = null, $slug)
	{
		$status = false;
		$send   = Payamito_Getway::instance()->payamito_group_send($to, $text, $sendernumber);
