	...
	class Payamito_Admin
	{
		...
		public function export()
		{
			...
			if (count($result) == 0) {
				...
				wp_send_json(['download' => false, 'messege' => __("There is no records", 'payamito'), 'payamito']);
			...
			wp_send_json(['file_name' => Payamito_Sent_List_Table::$file_name, 'download' => true, 'messege' => __("Downloading", 'payamito'), 'payamito']);
			die;
		}

		public function log_submenu()
		{
			add_submenu_page('payamito', __('Payamito logs', 'payamito'), __('Logs', 'payamito'), 'manage_options', 'payamito_logs', [&$this, 'render_table']);
		}
		...
		public function enqueue_scripts()
		{
			...
			if ($page === true) {
				...
				wp_localize_script('payamito-init-js', 'payanitoObject', array(
		...
		public function payamito_init_ajax()
		{
			...
			switch ($method) {
				case "statistics":
					...
					wp_send_json(array('statistics' => $statistics, 'crediet' => $crediet));
					break;
				case "connect":
					$crediet = payamito_code_to_message(payamito_get_crediet());
					...
					wp_send_json(array('connect' => $m, 'status' => $status, "title" => $title, "type" => $type, "btn" => __("OK", "payamito")));
					break;
			}
		}
				...
				if (is_array($data) && is_countable($data) && count($data) > 0) {
					switch ($day) {
						case -1:
							...
							break;
						case 0:
							$result[$day] = count(array_count_values($data)[date('Y-m-d', $current_time)]);
							break;
						default:
